...
Aqui você aprende um exemplo simples de **login com usuário e senha** no Streamlit. O app usa **Session State** para guardar se o usuário está “logado” ou não. As senhas são conferidas no próprio código (em um dicionário) – é um exemplo educativo. Em um app real, você não guardaria senhas no código; usaria um banco de dados ou um serviço de autenticação.
